...
from signature import filedialogdemo
from office import PDFMerge
...
class LeftTabWidget(QWidget):
    ...
    def initUi(self):
        ...
        for name, widget in self.MODULE.items():
            item = QListWidgetItem(
                QIcon('Data/0%d.ico' % randint(1, 8)), name, self.listWidget)
        ...
        #     widget = filedialogdemo()
            widgets = widget()
            # widgets.setStyleSheet('background: rgb(%d, %d, %d);margin: 50px;' % (
            #     randint(0, 255), randint(0, 255), randint(0, 255)))
            self.stackedWidget.addWidget(widgets)
